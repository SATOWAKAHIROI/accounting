...
export default function SubAccountList() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [subAccountsRes, accountsRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSubAccount) {
        await subAccountService.update(editingSubAccount.id, formData);
      } else {
  ...
  const handleDelete = async (subAccount) => {
    if (window.confirm(`${subAccount.name}を削除しますか？`)) {
      try {
        await subAccountService.delete(subAccount.id);
        fetchData();
      } catch (err) {
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-900">補助科目管理</h2>
          <button
              ...
              <form onSubmit={handleSubmit}>
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">
                    {editingSubAccount ? '補助科目編集' : '補助科目新規作成'}
                  </h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700">親科目</label>
                      <select
                        required
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">補助科目名</label>
                      <input
                        type="text"
                        required
                        value={formData.name}
